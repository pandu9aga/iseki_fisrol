...
<div class="container-fluid">
    <div class="card border-0 shadow-sm" style="border-radius: 12px;">
        <div class="card-header py-3 bg-white d-flex flex-column align-items-start"
            ...
            <h4 class="m-0 font-weight-bold" style="color: #e91e63;">Daily Temuan 5s</h4>
            <div class="d-flex align-items-center justify-content-between w-100 mt-3">
                <div>
                    <p class="m-0 font-weight-bold text-dark">
                        Name Daily Patrol : <span class="text-pink">{{ $daily_patrol->Name_Daily_Patrol ?? '-' }}</span>
                    </p>
            ...
            <button class="btn btn-primary mt-3" data-bs-toggle="modal" data-bs-target="#addDailyTemuanModal">
                ...
                <table id="example" class="table table-bordered datatable" style="font-size: 0.95rem; width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="py-3 px-4 text-pink font-weight-bold">{{ session('login_name') }}</td>
                            <td class="py-3 px-4">
                                @if ($temuan->Path_Daily_Temuan)
                                <img src="{{ asset('uploads/' . $temuan->Path_Daily_Temuan) }}" class="img-thumbnail"
                                    style="max-height:80px; object-fit: cover;">
                                @endif
                            </td>
                            <td class="py-3 px-4">{{ $temuan->Desc_Daily_Temuan }}</td>
                            <td class="py-3 px-4">
                                <button type="button" class="btn btn-sm btn-outline-success me-1 view-temuan"
                                    data-bs-toggle="modal" data-bs-target="#editDailyTemuanModal"
                                    data-id="{{ $temuan->Id_Daily_Temuan }}" data-foto-temuan="{{ $temuan->Path_Daily_Temuan }}"
                                    data-desc-temuan="{{ $temuan->Desc_Daily_Temuan }}">
                                    <i class="fas fa-eye"></i>
                                </button>
                                <form action="{{ route('user_daily_temuan.destroy', $temuan->Id_Daily_Temuan) }}" method="POST"
    ...
    <div class="modal-dialog">
        <div class="modal-content border-0"
            ...
            <div class="modal-header border-0 pb-0">
                <h5 class="modal-title" style="color: #e91e63;">Tambah Daily Temuan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body pt-0">
                <div class="mb-3">
                    <label class="form-label text-muted small">Penemu</label>
                ...
                <div class="mb-3">
                    ...
                    <input type="file" id="addFotoInput" class="form-control" accept="image/*" capture="environment" required>
                </div>
            ...
            <div class="modal-footer border-0 pt-0">
                ...
                <button type="button" class="btn btn-primary d-none" id="finalSaveTemuanBtn">Simpan Temuan</button>
            </div>
        </div>
    </div>
</div>

<!-- Edit Daily Temuan Modal -->
<div class="modal fade" id="editDailyTemuanModal" tabindex="-1" aria-labelledby="editDailyTemuanLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content border-0"
            style="border-radius: 12px; box-shadow: 0 4px 20px rgba(233, 30, 99, 0.15);">
            <div class="modal-header border-0 pb-0">
                <h5 class="modal-title" style="color: #e91e63;">Edit Daily Temuan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body pt-0">
                <div class="row mb-3">
                    <div class="col-md-12">
                        ...
                        <input type="file" id="editNewFotoInput" class="form-control mt-2 d-none" accept="image/*">
                    </div>
                </div>
                <div class="mb-3">
                    <label class="form-label text-muted small">Deskripsi Temuan</label>
            ...
            <div class="modal-body p-0 d-flex flex-column" style="min-height:0;">
                <div id="custom-tui-toolbar"
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <button type="button" class="btn btn-outline-primary" data-tool="rotate"><i class="fas fa-rotate"></i></button>
                    </div>
...
@section('style')
<style>
    .btn-pink { background: #e91e63; color: white; border: none; padding: 8px 16px; border-radius: 6px; font-weight: 500; }
    .btn-pink:hover { background: #d81b60; color: white; }
    .text-pink { color: #e91e63 !important; }
    .bg-light-pink { background-color: #fdf2f8 !important; }
    .card { border-radius: 12px; }
    .img-thumbnail { border-radius: 6px; border: 1px solid #eee; }
    .table thead th { font-weight: 600; font-size: 0.9rem; }
    #tui-editor-container { height: calc(100vh - 120px); }
    .tie-btn-history, .tie-btn-reset, .tie-btn-deleteAll, .tie-color-fill, .triangle, .circle, .tie-icon-add-button, .tui-image-editor-partition { display: none !important; }
    ...
    #custom-tui-toolbar .btn i { font-size: 1.3rem; }
    ...
    @media (max-width: 576px) {
        #custom-tui-toolbar { gap: 0.5rem !important; padding: 0.5rem !important; }
        #custom-tui-toolbar .btn {
        ...
        #custom-tui-toolbar .btn i { font-size: 1.4rem; }
        #custom-tui-toolbar .btn-success,
        #custom-tui-toolbar .btn-secondary {
    ...
    $(document).ready(function() {
            ...
            modal._element.addEventListener('shown.bs.modal', () => {
                ...
                tuiEditor.loadImageFromURL(imageUrl, 'uploaded').then(() => {
                    ...
                    fabric.Object.prototype.set({
                        borderColor: '#FF1493', cornerColor: '#FF1493', cornerStrokeColor: '#FF1493',
                        cornerSize: Math.round(20 * sc), transparentCorners: false, touchCornerSize: Math.round(28 * sc),
                        padding: Math.round(10 * sc), rotatingPointOffset: Math.round(55 * sc)
                    });
        ...
        $(document).on('click', '[data-tool]', function(e) {
            ...
            if (action === 'draw') {
                ...
                tuiEditor.setBrush({ width: toCanvas(10, canvas), color: '#FF1493' });
            } else if (action === 'rect') {
                ...
                canvas.renderAll();
            } else if (action === 'rotate') { tuiEditor.rotate(90);
            } else if (action === 'undo') { tuiEditor.undo();
            } else if (action === 'redo') { tuiEditor.redo();
            } else if (action === 'delete') { const active = canvas.getActiveObject(); if (active) { canvas.remove(active); canvas.renderAll(); } }
        });
        ...
        $(document).on('click', '#tui-save-btn', async function() {
            ...
            } else if ($('#editDailyTemuanModal').hasClass('show')) {
        ...
        $(document).on('click', '#finalSaveTemuanBtn', async function() {
            ...
            try {
                const res = await fetch("{{ route('user_daily_temuan.store', ['id' => $daily_patrol->Id_Daily_Patrol]) }}", {
                    method: 'POST', body: formData, headers: { 'X-CSRF-TOKEN': "{{ csrf_token() }}" }
                });
                const data = await res.json();
                if (data.success) {
                    bootstrap.Modal.getInstance(document.getElementById('addDailyTemuanModal')).hide();
                    location.reload();
                } else { console.error('Gagal menyimpan temuan:', data.message); }
            } catch (err) { console.error('Error menyimpan temuan:', err); }
        });
        ...
        $(document).on('click', '.view-temuan', function() {
            ...
            const fotoUrl = fotoPath ? "{{ asset('uploads') }}/" + fotoPath : "{{ asset('images/no-img.jpeg') }}";
        ...
        $(document).on('click', '#replaceFotoBtn', function() { $('#editNewFotoInput').click(); });
        ...
        $('#editNewFotoInput').on('change', async function(e) {
            ...
            try {
                ...
                const newFile = new File([resizedBlob], 'foto-temuan-' + Date.now() + '.jpg', { type: 'image/jpeg' });
                newFileImageData = newFile;
                $('#modalFotoTemuan').attr('src', URL.createObjectURL(newFile));
                editedImageData = null;
            } catch (err) { console.error('Gagal memuat gambar baru:', err); }
        });

        $(document).on('click', '#editFotoBtn', function() {
            const currentSrc = $('#modalFotoTemuan').attr('src');
            if (currentSrc && !currentSrc.includes('no-img.jpeg')) openTuiEditor(currentSrc, false);
        });

        $(document).on('click', '#updateTemuanBtn', async function() {
            const desc = $('#editDescTemuan').val().trim();
            if (!currentTemuanId || !desc) return;

            const formData = new FormData();
            formData.append('_method', 'PUT');
            formData.append('Desc_Daily_Temuan', desc);

            if (newFileImageData) formData.append('Path_Daily_Temuan', newFileImageData);
            else if (editedImageData && editedImageData.startsWith('data:image')) formData.append('Path_Daily_Temuan', editedImageData);

            try {
                const res = await fetch("{{ route('user_daily_temuan.update', ['id' => '__ID__']) }}".replace('__ID__', currentTemuanId), {
                    method: 'POST', body: formData, headers: { 'X-CSRF-TOKEN': "{{ csrf_token() }}" }
                });
                const data = await res.json();
                if (data.success) { bootstrap.Modal.getInstance(document.getElementById('editDailyTemuanModal')).hide(); location.reload(); }
                else { console.error('Gagal memperbarui temuan:', data.message); }
            } catch (err) { console.error('Error memperbarui temuan:', err); }
        });
        ...
        $(document).on('click', '.delete-temuan', function() {
            ...
            fetch("{{ route('user_daily_temuan.destroy', ['id' => '__ID__']) }}".replace('__ID__', id), { method: 'POST', body: formData })
                .then(response => response.json())
                .then(data => { if (data.success) location.reload(); else console.error('Gagal menghapus:', data.message); })
        ...
        if (!$.fn.DataTable.isDataTable('#example')) {
            $('#example').DataTable({
                ...
                "language": { "search": "Cari:", "lengthMenu": "Tampilkan _MENU_ data per halaman", "info": "Menampilkan _START_ sampai _END_ dari _TOTAL_ data", "infoEmpty": "Tidak ada data", "infoFiltered": "(difilter dari _MAX_ total data)", "zeroRecords": "Tidak ditemukan data yang sesuai", "paginate": { "first": "Pertama", "last": "Terakhir", "next": "»", "previous": "«" } },
